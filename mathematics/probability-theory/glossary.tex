...
{%
  A \gls{probability distribution} in the form of $P(X\mid Y)$ where for each value of $Y$ this object assigns a probability over values of $X$ using the \gls{conditional probability}. It is only defined for non-zero $P(X=x)$. $Y$ represents \textit{prior} knowledge.
}
...
\longnewglossaryentry{set of random variables}{name=set of random variables}
{%
  Notation:
  \begin{itemize}
